      ...
      .subscribe(r => {
        if (r.success) {
          ...
          if (unseenIds.length > 0) {
            // After 3s: mark as read in backend + remove nav badge (cancelled if user navigates away)
            this._seenTimer = setTimeout(() => {
              this.#http.patch<StandardResponse<null>>(
                environment.SERVER_URL + '/comments/mark-seen', { ids: unseenIds }
              ).subscribe();
              this.$datesWithUnseenComments.update(dates => dates.filter(d => d !== date));
            }, 3000);
            // After 20s: remove inline unread icon (runs to completion regardless of navigation)
            setTimeout(() => {
              this.$comments.update(cs => cs.map(c => unseenIds.includes(c._id) ? { ...c, seen: true } : c));
            }, 20000);
          }
        }
      });
